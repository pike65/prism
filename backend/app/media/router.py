...
def create_item(item: schemas.MediaCreateRequest, db: Session = Depends(get_db)):
    return service.add_new_media(db=db, item_data=item)
...
def update_item(
    ...
    updated_media = service.update_media(db=db, item_id=media_id, item_data=item)
